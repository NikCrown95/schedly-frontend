...
export default function Registrati(){
  ...
  return <><main className="mobileMock mRegisterPage">
    ...
    <form className="mLoginForm">
      <label>Nome e cognome<input type="text" name="name" placeholder="Mario Rossi" autoComplete="name" required/></label>
      <label>Email<input type="email" name="email" placeholder="mario@email.it" autoComplete="email" required/></label>
      <label>Password<div className="mPasswordWrap"><input type={showPassword?"text":"password"} name="password" placeholder="Crea una password" autoComplete="new-password" required/><button className="mEye" type="button" aria-label={showPassword?"Nascondi password":"Mostra password"} aria-pressed={showPassword} onClick={()=>setShowPassword(v=>!v)}><svg viewBox="0 0 24 24"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.7"/></svg></button></div></label>
      <button className="mBtn" type="submit">Registrati →</button>
